...
//    sum(j in G)sum( m in numberOfEVs) sum(i in G :i!=j)x[<i,j,m>] == sum(j in G)sum(k in G :k!=j)x[<j,k,m>];
  ...
  forall(i,j in G, m in numberOfEVs :  i!=j){
    demandAccomadationInVehiclePt2: 
    load[<i,j>] 
    <= 
    (vehicleCapacity[m] * 
    x[<i,j,m>]) - 
  ...
  forall(k,i in G, j in chargingStationSet, m in numberOfEVs : i!=j && j!=k && k!=i){
    chargingStationTimeManagementPt2:     
    departureTime[<k,i>] + 
    travelTime[<i,j>] + 
    ((batteryCapacity[m] - remainingBatteryCapacity[<i,j,m>])/rechargingRate) - departureTime[<i,j>]
    <=  
  ...
  forall(i in chargingStationSet, j in vertexSet, m in numberOfEVs : i!=j){
    batteryFullAtDepartureOfChargingStationPt2:    
    remainingBatteryCapacity[<i,j,m>] - 
    (batteryCapacity[m] 
    - 
    energyConsumption[<i,j>])
  ...
  forall(i in customerSet, k,j in G, m in numberOfEVs : i!=j && j!=k && k!=i){    
    batteryFullAtDepartureOfChargingStationPt3:
    batteryCapacity[m]
    *
    (x[<i,j,m>] -1)
    <= 
  ...
  forall(i in customerSet, k,j in G, m in numberOfEVs : i!=j && j!=k && k!=i){
    batteryFullAtDepartureOfChargingStationPt4:
    remainingBatteryCapacity[<i,j,m>] - 
    (remainingBatteryCapacity[<k,i,m>] - 
    energyConsumption[<i,j>]) 
    <= 
  ...
  forall(i,j in G, m in numberOfEVs : i!=j){
    remainingBatteryWithinCapacityPt2:  
    remainingBatteryCapacity[<i,j,m>] 
    <=
    batteryCapacity[m];
  }
...
//  forall(j in chargingStationSet, i in G, m in numberOfEVs: i!=j)
//    chargingStationRecharge:
//    remainingBatteryCapacity[<i,j,m>] == batteryCapacity[m];
